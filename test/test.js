...
describe('live test', function () {
  ...
  before(function (done) {
    if (!process.env.LD_APITOKEN || !process.env.LD_PRJKEY || !process.env.LD_SRCENVKEY || !process.env.LD_DESTENVKEY) {
      this.skip();
    } else {
        ...
        flags.forEach(function (flag) {
          describe('for the `' + flag.key + '` flag', function () {
            it('the `on` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].on,
                flag.environments[process.env.LD_DESTENVKEY].on,
              );
            });
            it('the `archived` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].archived,
                flag.environments[process.env.LD_DESTENVKEY].archived,
              );
            });
            it('the `targets` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].targets,
                flag.environments[process.env.LD_DESTENVKEY].targets,
              );
            });
            it('the `rules` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].rules,
                flag.environments[process.env.LD_DESTENVKEY].rules,
              );
            });
            it('the `prerequisites` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].prerequisites,
                flag.environments[process.env.LD_DESTENVKEY].prerequisites,
              );
            });
            it('the `fallthrough` value should be copied', function () {
              assert.deepEqual(
                flag.environments[process.env.LD_SRCENVKEY].fallthrough,
                flag.environments[process.env.LD_DESTENVKEY].fallthrough,
              );
            });
          });
        });
